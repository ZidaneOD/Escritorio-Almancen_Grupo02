...
public class CMCambiarAlmacen implements CICambioAlmacen<CEAlmacen> {
    ...
    public void saveAlmacenCIC(CEAlmacen objObjeto) throws Exception {
        ...
        cs.setInt(2, objObjeto.getIdUbicacion());
        cs.execute();
    }
    ...
    public void modificarAlmacenCIC(CEAlmacen objObjeto) throws Exception {
        ...
        CallableStatement cs = cn.prepareCall(sql);
        cs.setInt(1, objObjeto.getIdAlmacen());
        cs.setString(2, objObjeto.getNombAlm());
        cs.setInt(3, objObjeto.getIdUbicacion());
        cs.execute();
    }
    ...
    public ResultSet buscar(Object objObject) throws Exception {
        ...
        String sql = "select * from valmacen where almacen like ?";
    ...
    public ResultSet mostrar() throws Exception {
        ...
        Connection cn = ConMySQL.getInstance().getConnection();
        String sql = "select * from valmacen";
        PreparedStatement ps = cn.prepareStatement(sql, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
        ResultSet rs = ps.executeQuery();
        return rs;
    }

    public ResultSet buscarEstado(Object objObject) throws Exception {

        Connection cn = ConMySQL.getInstance().getConnection();
        String nombre = "%" + objObject + "%";
        String sql = "select * from vestado where nombesta like ?";
        PreparedStatement ps = cn.prepareStatement(sql, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
        ps.setString(1, nombre);
        ResultSet rs = ps.executeQuery();
        return rs;
